...
import { isAddress, parseUnits, toQuantity } from 'ethers'
...
const MOCK_BALANCE = parseUnits('100', 'ether')
...
export async function simulate({
  ...
  await forkProvider.send('tenderly_addBalance', [
    [treasuryAddress],
    toQuantity(MOCK_BALANCE),
  ])

  let totalGasUsed = constants.Zero
  ...
  for (let i = 0; i < targets.length; i++) {
    const txParams = {
      ...
      value: toQuantity(BigNumber.from(values[i]).toHexString()),
      data: calldatas[i],
    }
    ...
    const forkViewRes = (
      await axios.get(`${TENDERLY_FORK_V2_BASE_URL}/${forkId}`, opts)
    ).data
  ...
  const finalBalance = BigNumber.from(balanceResponse)
    .add(totalGasUsed)
    .sub(MOCK_BALANCE)
