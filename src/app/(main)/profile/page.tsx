...
export const generateStaticParams = async (): Promise<{ user: string }[]> => {
  const users = [
    'agcook.eth',
    'dannylharle.eth',
    'planet1999.eth',
...
const UserPage = ({ params, searchParams }: UsersProfileProps): JSX.Element => {
  ...
  const { user } = params
  const currentUser = USERS_PROFILE_DATA.find(
